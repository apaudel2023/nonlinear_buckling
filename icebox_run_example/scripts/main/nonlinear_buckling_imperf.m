...
% === Load configuration ===
config = cfg_nonlinear_buck_imperf();  % You need to create this config
...
for itk = config.sample_start:config.sample_end
    ...
    % Create imperfection and load files
    impf_dat_file = 'INCLUDE_IMPF_LBA_MAIN.dat';
    SOL_400_NONLINEAR_BUCK_FUNC_MAIN(sf_value, op2_input_file, impf_dat_file);
    LOADS_FUNC(Pcr, 'INCLUDE_LOAD_APPLIED_LBA_IMPF.dat');

    %% === Run Nonlinear SOL 400 ===
    bdf_file_400 = 'BDF_IMPERFECT_NON_LINEAR_SOL_400.bdf';
    ...
    fprintf('[INFO] Running SOL 400 Nonlinear...\n');
    system(cmd400);

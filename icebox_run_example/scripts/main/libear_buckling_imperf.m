...
for itk = config.sample_start:config.sample_end
    ...
    eig_path = fullfile(temp_dir, f06_file_105);
    if isfile(eig_path)
    ...
    SOL_400_IMPF_LBA_FUNC_MAIN(sf_value, op2_file_105, impf_dat_file);

    %% === Copy files to SOL400 dir ===
    sol400_dir = fullfile(temp_dir, 'SOL400');
    mkdir_if_missing(sol400_dir);

    copyfile(op2_file_105, fullfile(sol400_dir, op2_file_105));
    copyfile('COMPOSITE_MAT_PROP1.dat', fullfile(sol400_dir, 'COMPOSITE_MAT_PROP1.dat'));
    copyfile('COMPOSITE_MAT_PROP2.dat', fullfile(sol400_dir, 'COMPOSITE_MAT_PROP2.dat'));
